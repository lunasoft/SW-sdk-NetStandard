...
{
    public class StampV2 : BaseStamp
    {
        BuildSettings Build = new BuildSettings();
        SW.Services.Stamp.StampV2 stamp;

        public StampV2(bool isToken = false)
        {
            if (isToken)
                stamp = new SW.Services.Stamp.StampV2(Build.Url, Build.Token);
            else
                stamp = new SW.Services.Stamp.StampV2(Build.Url, Build.User, Build.Password);
        }
